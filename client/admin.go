...
func (a *App) ComplaintList() (netproto.Complaints, error) {
	f, err := a.request(netproto.MsgComplaintList, netproto.MsgComplaints,
...
func (a *App) ComplaintClear(targetUniqueID, fromUniqueID string) (netproto.Complaints, error) {
	f, err := a.request(netproto.MsgComplaintClear, netproto.MsgComplaints,
...
func (a *App) tokenRequest(mt netproto.MessageType, msg any) (netproto.Tokens, error) {
	f, err := a.request(mt, netproto.MsgTokens, msg, 5*time.Second)
	if err != nil {
...
func (a *App) TokenUse(token string) string {
	if err := a.write(netproto.MsgTokenUse, netproto.TokenUse{Token: token}); err != nil {
		return err.Error()
	}
	return ""
}
